...
class Meta_Prototype_3d(nn.Module):
    def __init__(self,
                 proto_size,
                 feature_dim,
                 key_dim,
    ...
    def forward(self, key, query, weights, train=True):
        ...
        # test
            ...
            # loss
            updated_query, fea_loss, query = self.query_loss(
                query, protos, weights, train)
    ...
    def query_loss(self, query, keys, weights, train):
        ...
        if train:
            ...
            dis_loss = dis.sum(1).sum(1) * 2 / (self.proto_size *
                                                (self.proto_size - 1))
            ...
            keys = F.normalize(keys, dim=-1)
            softmax_score_query, softmax_score_proto = self.get_score(
                keys, query)
