...
class DataLoader(data.Dataset):
    ...
    def get_all_samples(self):
        ...
        for video in sorted(videos):
            ...
            video_name = video.split('/')[-1]
            for i in range(
                    len(self.videos[video_name]['frame']) - self._time_step):
...
class MetaDataLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        frame_ind = random.sample(range(0, length // self.num_segs),
                                  self.task_size)
        ...
        for j in range(self.task_size):
            ...
            batch.append(np.expand_dims(np.concatenate(couple, axis=0),
                                        axis=0))
        # import pdb;pdb.set_trace()
        return np.concatenate(batch, axis=0)
