...
def calc(r=5, sigma=2):
    k = np.zeros(r)
    for i in range(r):
        k[i] = 1 / ((2 * math.pi)**0.5 * sigma) * math.exp(-(
            (i - r // 2)**2 / 2 / (sigma**2)))
    return k
...
def point_score(outputs, imgs):
    ...
    score = (torch.sum(normal * loss_func_mse(
        (outputs[0] + 1) / 2, (imgs[0] + 1) / 2)) / torch.sum(normal)).item()
    return score
...
def anomaly_score_list(psnr_list):
    ...
    for i in range(len(psnr_list)):
        anomaly_score_list.append(
            anomaly_score(psnr_list[i], np.max(psnr_list), np.min(psnr_list)))
...
def anomaly_score_list_inv(psnr_list):
    ...
    for i in range(len(psnr_list)):
        anomaly_score_list.append(
            anomaly_score_inv(psnr_list[i], np.max(psnr_list),
                              np.min(psnr_list)))
...
def depict(videos_list,
    ...
    for video in sorted(videos_list):
        ...
        anomaly_score_ae_list = np.asarray(
            anomaly_score_list(psnr_list[video_name]))
        anomaly_score_mem_list = np.asarray(
            anomaly_score_list_inv(feature_distance_list[video_name]))
        if (1 - labels_list[start:end]).max() < 1 or (
                1 - labels_list[start:end]).min() == 1:
            accuracy_ae = accuracy_me = 0
        else:
            accuracy_ae = AUC(anomaly_score_ae_list,
                              np.expand_dims(1 - labels_list[start:end], 0))
            accuracy_me = AUC(anomaly_score_mem_list,
                              np.expand_dims(1 - labels_list[start:end], 0))
        ...
        plt.title(video_name +
                  ' {:.4f} {:.4f}'.format(accuracy_ae, accuracy_me),
                  y=3.4)
        plt.show()
