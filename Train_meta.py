...
parser.add_argument('--loss_fra_reconstruct',
                    type=float,
                    default=1.00,
                    help='weight of the frame reconstruction loss')
parser.add_argument('--loss_fea_reconstruct',
                    type=float,
                    default=1.00,
                    help='weight of the feature reconstruction loss')
parser.add_argument('--loss_distinguish',
                    type=float,
                    default=0.0001,
                    help='weight of the feature distinction loss')
...
parser.add_argument('--resume',
                    type=str,
                    default='exp/pretrain_model.pth',
                    help='file path of resume pth')
...
parser.add_argument(
    ...
    default=4,
    help=
    'task size for meta training, 1 for meta training, the others for meta validation'
)
parser.add_argument(
    '--segs',
    type=int,
    default=32,
    help='number of segs when dividing the videos for meta training')
parser.add_argument('--meta_base',
                    type=str,
                    default='exp/pretrain_model.pth',
                    help='file path of resume pth')
parser.add_argument('--meta_init_lr',
                    type=float,
                    default=1e-6,
                    help='meta initial learning rate')
parser.add_argument('--meta_alpha_init',
                    type=float,
                    default=1e-6,
                    help='meta initial alpha')
parser.add_argument('--meta_alpha_lr',
                    type=float,
                    default=1e-6,
                    help='meta initial alpha learning rate')
parser.add_argument('--meta_epoch',
                    type=int,
                    default=1000,
                    help='meta training epoch')
...
# Loading dataset
train_dataset = MetaDataLoader(train_folder,
                               transforms.Compose([
                                   transforms.ToTensor(),
                               ]),
...
for epoch in range(start_epoch, args.meta_epoch):
    ...
    for j, (imgs) in enumerate(train_batch):
        ...
        for k in range(args.batch_size):
            ...
            init_g, alpha_g, loss[k], lh_loss[k], idx = train_init(
                model, meta_init, meta_alpha, loss_func_mse,
                imgs[k][:args.task_size // 2, :12],
                imgs[k][args.task_size // 2:, :12],
                imgs[k][:args.task_size // 2,
                        12:], imgs[k][args.task_size // 2:, 12:], idx, args)
        ...
        meta_update(model, meta_init, meta_init_grads, meta_alpha,
                    meta_alpha_grads, meta_init_optimizer,
                    meta_alpha_optimizer)

        pbar.set_postfix({
            'Mpoch':
            '{0} {1}'.format(epoch + 1, args.exp_dir),
            'Lr':
            '{:.6f},{:.6f}'.format(
                meta_init_optimizer.param_groups[-1]['lr'],
                meta_alpha_optimizer.param_groups[-1]['lr']),
            'loss':
            '{:.6f}({:.4f})'.format(loss.mean().item(), loss_raw.avg),
            'lh_loss':
            '{:.6f}({:.4f})'.format(lh_loss.mean().item(), loss_lh.avg),
        })
        pbar.update(1)

    print('----------------------------------------')
    print('Mpoch:', epoch + 1)
    print('Lr: {:.6f}'.format(meta_init_optimizer.param_groups[-1]['lr'],
                              meta_alpha_optimizer.param_groups[-1]['lr']))
