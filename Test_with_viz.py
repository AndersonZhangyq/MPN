...
parser.add_argument('--K_hots',
                    type=int,
                    default=0,
                    help='number of the K hots')
...
print('Evaluation of Version {0} on {1}'.format(
    args.model_dir.split('/')[-1], args.dataset_type))
...
pbar = tqdm(
    ...
    bar_format=
    '{l_bar}|{bar}| {n_fmt}/{total_fmt} [{rate_fmt}{postfix}|{elapsed}<{remaining}]',
)
...
with torch.no_grad():
    for k, (imgs) in enumerate(test_batch):
        ...
        for j in range(len(mse_imgs)):
            psnr_score = psnr(mse_imgs[j].detach().item())
            fea_score = psnr(mse_feas[j].detach().item())
        ...
        pbar.set_postfix({
            ...
            '{0}'.format(ckpt_name),
            'Vid':
            '{0}'.format(args.dataset_type + '_' +
                         videos_list[vid].split('/')[-1]),
            'AEScore':
            '{:.6f}'.format(psnr_score),
            'MEScore':
            '{:.6f}'.format(fea_score),
            'time':
            '{:.6f}({:.6f})'.format(end_t - start_t, forward_time.avg),
        })
        pbar.update(1)
...
anomaly_score_per_video = OrderedDict()
for video in sorted(videos_list):
    video_name = video.split('/')[-1]
    template = calc(15, 2)
    aa = filter(anomaly_score_list(psnr_list[video_name]), template, 15)
    bb = filter(anomaly_score_list(feature_distance_list[video_name]),
                template, 15)
    anomaly_score_per_video[video_name] = score_sum(aa, bb, args.alpha)
...
accuracy_total = 100 * AUC(anomaly_score_total,
                           np.expand_dims(1 - labels_list, 0))

with open(os.path.join(psnr_dir, "log.txt"), "w+") as f:
    f.write('Total AUC: {:.4f}%'.format(accuracy_total))
    f.flush()

print('The result of Version {0} Epoch {1} on {2}'.format(
    psnr_dir.split('/')[-1], ckpt_name, args.dataset_type))
